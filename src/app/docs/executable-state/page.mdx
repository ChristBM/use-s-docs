# Executable State (For deep-divers only)
...
> ⚠️ **Note:** `useS v2.1.0` **Does** compare function-type values.
> In most cases, if function memory references are not handled correctly,
> the comparison will likely result in them being different, causing a re-render.
> This behavior is consistent with React's default handling of function references.
